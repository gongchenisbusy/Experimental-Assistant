# Experimental Assistant v1.1.0 中文快速入门
...
v1.1.0 可以按用户提出的任意数据类别建立模式，并从用户合法获得的可搜索全文中收集相应数据。字段可为数值、范围、带不确定度数值、文本、枚举、布尔、日期、列表或嵌套结构，并保留页码/表格/图注/短上下文证据。用户可以逐条接受、拒绝、编辑、推迟或标记为不可比较；只有接受或编辑后的记录可以进入统计、作图、报告或导出。
