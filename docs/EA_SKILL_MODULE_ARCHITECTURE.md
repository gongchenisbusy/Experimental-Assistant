...
这些建议进入分析或报告前必须保留来源、适用性说明、置信度、不确定性和审核状态。若建议会改变确定性数据处理、拟合约束或项目记忆，EA 必须先把来源和参数记录成可追溯的 suggestion/source packet，并在用户或工作流审核后再应用。EA 的边界是禁止无来源常数、静默套用参数、把候选解释写成证明性结论，而不是禁止有科学依据的主动建议。
