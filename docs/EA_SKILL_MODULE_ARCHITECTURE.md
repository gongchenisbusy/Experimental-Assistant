...
### 2.3.1 Source-Backed 建议原则

EA 的谨慎边界不应理解为“不使用外部科学知识”。当项目背景、样品体系、表征手段和用户目标足够明确时，EA 可以主动从本地文献库、项目参考记录、用户提供来源或用户确认的网络检索中整理候选参数、峰位/谱带归属、模型假设、拟合窗口和后续实验建议。

这些建议进入分析或报告前必须保留来源、适用性说明、置信度、不确定性和审核状态。若建议会改变确定性数据处理、拟合约束或项目记忆，EA 必须先把来源和参数记录成可追溯的 suggestion/source packet，并在用户或工作流审核后再应用。EA 的边界是禁止无来源常数、静默套用参数、把候选解释写成证明性结论，而不是禁止有科学依据的主动建议、文献查找或候选化学态讨论。
...
Thermal analysis 当前只覆盖 TGA/DSC/DTG 风格表格数据的 review-gated first-pass workflow，Tg/Tm/Tc、基线模型、动力学、重复性统计和正式热稳定性判断仍应由后续协议化模块实现。Electrochemistry 当前覆盖 CV/LSV/chrono/GCD 风格表格数据的 review-gated first-pass workflow、EIS Nyquist screening、用户确认 frequency/model/initial values/bounds 的 EIS series-R-RC circuit-fit screening、用户确认 offset 的 potential conversion 坐标换算、用户确认 Ru/fraction/sign convention 的 iR drop coordinate correction、用户确认 kinetic window 的 Tafel/overpotential screening fit，以及用户确认 discharge window/current/mass/area/loading 的 GCD capacity/capacitance screening metrics；自动 circuit/model 选择、自动 kinetic/discharge-window 选择、倍率/循环稳定性和正式性能指标仍应由后续协议化模块实现。更复杂的显微/散射模块可继续进入设计和接口测试，不必一次性完整实现。
