...
- `batch-comparison`: 多样品、多批次统计比较；v0.2 先提供 Raman/PL/XRD/FTIR 已确认条目的批处理协调和汇总，不替代每个表征方法的 review gates。
...
- `raman-analysis`、`pl-analysis`、`xrd-analysis` 和 `ftir-analysis` 作为第一批可运行表征 workflow 样例。
- `local-literature-library` 作为项目初始化时的知识基础样例。

UV-Vis、XPS、电化学、热分析和更复杂的显微/散射模块可先进入设计和接口测试，不必一次性完整实现。
