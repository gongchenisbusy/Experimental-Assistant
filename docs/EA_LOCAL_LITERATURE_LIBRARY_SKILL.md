...
- 若用户提供 Clarivate Journal Citation Reports、SJR/CiteScore 或其他权威表格，或专用文献 workflow 在用户确认的检索/账号环境中记录了可追溯来源，EA 可使用具体影响因子或 venue metric。
- 若没有可追溯来源，EA 不应伪造影响因子；可使用期刊层级、出版社、引用数、领域声誉、SJR/CiteScore 等可得 proxy，并明确标注来源和 proxy 属性。
...
- 影响因子不是完全开放的实时数据源，不能在无来源时硬编码或臆造；若通过用户提供、用户确认的专用文献 workflow 或公开指标获得，必须把来源和获取方式写入记录。
