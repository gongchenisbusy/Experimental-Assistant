...
Raw files should be imported as controlled project copies before processing. Experimental Assistant v1.1.0 currently has concrete workflows for Raman, PL, XRD, FTIR, UV-Vis, XPS, electrochemistry, thermal analysis, and image-style characterization records.
...
ea raman list-assignment-libraries \
  --material mos2 \
  --feature mos2_a1g_like \
  --shift-min-cm1 400 \
  --shift-max-cm1 420
...
For UV-Vis source-backed interpretation work, use `ea uv-vis list-source-libraries` first when a user or agent needs to discover built-in source-backed starter candidates, candidate IDs, candidate types, optical targets, energy/wavelength ranges, source summaries, reference seeds, filters, and next commands before choosing a source packet. The bundled `generic_optical_interpretation` library includes low-confidence starters for direct/indirect Tauc-style screening context, a broad wide-band-gap oxide edge discussion window, visible sub-band-gap feature discussion, and diffuse-reflectance/Kubelka-Munk context. Use `ea uv-vis build-source-packet --builtin-library generic_optical_interpretation` for the bundled starter path, `--library-file` or `--write-template` for project-local libraries and editable templates, or `--literature-manifest literature/confirmed_uv_vis_source_candidates.yml` for a confirmed local literature/source-candidate manifest. Discovery and packet-building are deterministic local steps only: they do not run live search, download or parse articles, register references, inject citations, apply optical models/corrections, create ReviewRecords, write memory, or prove band gaps, transition mechanisms, feature assignments, or correction validity. Use `ea uv-vis suggest-interpretations` only after a source packet and processed UV-Vis metadata exist; use `ea uv-vis prepare-review`, a confirmed `uv_vis_interpretation_suggestions` ReviewRecord, and registered references before report or memory reuse.
...
ea uv-vis list-source-libraries \
  --builtin-library generic_optical_interpretation \
  --candidate-type optical_gap_candidate \
  --optical-target oxide \
  --energy-min-ev 3.0 \
  --energy-max-ev 3.4
ea uv-vis build-source-packet /path/to/ea-project \
  ...
  --candidate-type optical_gap_candidate \
  --optical-target oxide \
  --output suggestions/uv_vis/source-packets/uv_vis_builtin_source_packet.yml
ea uv-vis build-source-packet /path/to/ea-project \
...
Before sharing an Experimental Assistant v1.1.0 repository package:
...
ea-sign-release-package dist/experimental-assistant-1.1.0-COMMIT-release.zip \
  --private-key /path/to/user-release-private.pem \
  --public-key /path/to/user-release-public.pem

ea-verify-release-signature dist/experimental-assistant-1.1.0-COMMIT-release.zip \
  --public-key /path/to/user-release-public.pem
```
