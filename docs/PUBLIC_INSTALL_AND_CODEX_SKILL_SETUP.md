...
Product identity: `Experimental Assistant v0.9.6` with package compatibility name `ea-v0-2`. The Python package name and Codex skill folder intentionally remain `ea-v0-2` for compatibility with existing projects and skill installs. That name is not the public release version.
...
```text
Installed Experimental Assistant v0.9.6.
Package compatibility name: ea-v0-2 0.9.6.
Codex invocation: $ea-v0-2
Restart Codex before using this skill in a new thread.
```

After restarting Codex, open a new thread and invoke EA as `$ea-v0-2`. This is the compatibility skill name for Experimental Assistant v0.9.6.
...
- locates the local checkout skill or fetches the `v0.9.6` skill from GitHub;
...
Use the GitHub Release zip when you need a fixed handoff package rather than a git checkout. Download `ea-v0-2-0.9.6-COMMIT-release.zip` and its `.sha256` sidecar from the release page, verify them, then install from the extracted folder:

```bash
shasum -a 256 -c ea-v0-2-0.9.6-COMMIT-release.zip.sha256
python3 -m venv .venv
. .venv/bin/activate
python3 -m pip install -e .
ea codex install-skill --source skills/ea-v0-2
...
Experimental Assistant v0.9.6 uses the `ea-v0-2` compatibility skill folder. Older `ea-v0-1` skills may remain installed for old workflows. The v0.9.6 installer does not remove `ea-v0-1` and does not modify existing project folders.

Use `$ea-v0-2` for new Experimental Assistant v0.9.6 work. Existing project folders from the EA compatibility line can be inspected with `ea healthcheck` and `ea eval project`.
