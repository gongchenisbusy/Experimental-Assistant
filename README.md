...
Electrochemistry processing supports reviewed `measurement_mode=eis` for two-column Nyquist screening with `x_unit=ohm` and `current_unit=unknown`; the output is a traceable impedance summary, not equivalent-circuit fitting or an Rct/mechanism claim. Electrochemistry templates also include disabled-by-default `correction_record` settings for reviewed reference-electrode, converted-scale, uncompensated-resistance, and iR-compensation metadata, plus disabled-by-default `potential_conversion` settings for user-reviewed offset conversion that writes a converted potential column and record. Correction records are provenance only; potential conversion is a coordinate transform only and does not apply numeric iR correction, Tafel/GCD analysis, equivalent-circuit fitting, or performance/mechanism claims.
