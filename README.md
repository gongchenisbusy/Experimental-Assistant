...
UV-Vis templates include disabled-by-default `tauc_analysis` and `derivative_analysis` settings; enable them only after the user reviews the transform/transition/window or derivative axis. The resulting Tauc/Kubelka-Munk intercepts and derivative extrema are screening records, not definitive band-gap, transition, or mechanism assignments.
Electrochemistry processing supports reviewed `measurement_mode=eis` for two-column Nyquist screening with `x_unit=ohm` and `current_unit=unknown`; the output is a traceable impedance summary, not equivalent-circuit fitting or an Rct/mechanism claim.
