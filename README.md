...
UV-Vis templates include disabled-by-default `tauc_analysis` settings; enable them only after the user reviews the transform, transition/exponent, and `fit_window_eV`. The resulting Tauc/Kubelka-Munk intercept is a screening record, not a definitive band-gap assignment.
