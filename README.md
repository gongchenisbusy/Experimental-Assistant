...
Thermal templates include disabled-by-default `context_record` settings for reviewed DSC sign convention, baseline/reference, sample, atmosphere/program, and correction-note metadata. Thermal context records are metadata/provenance only and do not automatically invert DSC signs, apply baseline/reference correction, assign Tg/Tm/Tc, fit kinetics, or prove decomposition/melting/crystallization mechanisms.
