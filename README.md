...
Thermal templates include disabled-by-default `baseline_correction` settings for reviewed DSC/DTG linear baseline processing and disabled-by-default `context_record` settings for reviewed DSC sign convention, baseline/reference, sample, atmosphere/program, and correction-note metadata. Thermal baseline correction is a numeric processing step only, and thermal context records are metadata/provenance only; neither automatically assigns Tg/Tm/Tc, fits kinetics, ranks thermal stability, or proves decomposition/melting/crystallization mechanisms.
