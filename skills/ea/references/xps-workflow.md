...
Use this reference when processing X-ray photoelectron spectroscopy spectra in the Experimental Assistant v0.9.7 compatibility skill.
...
5. If source-backed XPS parameter suggestions are useful, EA may prepare candidate parameters from built-in seed libraries such as `generic_xps_parameters` and the optional `oxide_o1s_binding_energy` O 1s/oxide-surface pack, a reviewed local library, project literature record, user-provided source, public database, or user-confirmed/agent-led literature-search workflow. This does not require every signed energy separation, area ratio, FWHM ratio, Tougaard parameter, reviewed bound, or binding-energy/chemical-state candidate to be typed by the user; source-backed values can be proposed when element/region/sample context, source, applicability, and calibration/charge-reference assumptions are recorded. Use `ea xps list-parameter-libraries` to inspect built-in candidate coverage and reference seeds before deciding which packet to build. Use `ea xps build-source-packet` with no library arguments for the default built-in starter packet, `--builtin-library generic_xps_parameters` for explicit default selection, `--builtin-library oxide_o1s_binding_energy` when an O 1s oxide-surface candidate pack is useful, `--library-file`/`--write-template` for project-local libraries and editable templates, or `--literature-manifest` for a confirmed literature/source-candidate manifest; then run `ea xps suggest-parameters` on the source packet before copying any values into processing parameters or discussing binding-energy candidates in reports/memory. Use `ea xps prepare-review` to create a grouped YAML/Markdown review package, then ask the user to review candidate values and register missing references. The no-live-lookup command boundary is not a ban on EA looking up source-backed parameters; it prevents hidden lookup/download/full-text parsing inside deterministic processing or suggestion-record commands.
...
Current Experimental Assistant v0.9.7 XPS compatibility support:
...
ea xps build-source-packet /path/to/ea-project --builtin-library generic_xps_parameters --suggestion-type binding_energy_candidate --output suggestions/xps/source-packets/xps_binding_energy_source_packet.yml
...
```bash
ea xps list-parameter-libraries --builtin-library generic_xps_parameters --element Fe --core-level 2p
...
ea xps build-source-packet /path/to/ea-project --builtin-library oxide_o1s_binding_energy --include-candidate xps-builtin-o1s-lattice-oxide-binding-energy-candidate --output suggestions/xps/source-packets/o1s_oxide_packet.yml
