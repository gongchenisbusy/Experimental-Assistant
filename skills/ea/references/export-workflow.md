...
For batch bundles, `--include-trace` passes focused trace view generation into each nested report bundle. The top-level batch manifest records `trace_export.strategy: nested_report_focused_trace_views`; Experimental Assistant v0.9.7 does not emit a batch-level trace graph until the traceability service models batch nodes.
