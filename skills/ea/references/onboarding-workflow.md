...
When updating a verified public v0.9.9 installation to v1.1.0, preview `ea update --release-ref v1.1.0`, confirm with `--yes`, then run `ea version`, `ea doctor`, and `ea install-check`. Use `ea rollback --release-ref v0.9.9` for a read-only rollback plan before any explicit restore.

The onboarding message should identify Experimental Assistant v1.1.0, present `$ea` as the only skill invocation, and remind the user to restart Codex after skill installation or replacement.
