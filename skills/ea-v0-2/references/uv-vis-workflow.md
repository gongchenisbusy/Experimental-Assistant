...
Use this reference when processing UV-Vis absorbance, transmittance, or reflectance spectra in the Experimental Assistant v0.9.6 compatibility skill.
...
Current Experimental Assistant v0.9.6 UV-Vis compatibility support:
...
- Processing supports optional Savitzky-Golay smoothing, max-absolute normalization, SciPy optical feature detection, a low-confidence normalized-threshold optical edge estimate, reviewed-parameter Tauc/Kubelka-Munk screening, disabled-by-default derivative screening, disabled-by-default correction-context records, and disabled-by-default reviewed numeric correction.
- `signal_mode=absorbance` detects maxima; `signal_mode=transmittance` or `reflectance` detects minima.
- Processed CSV files include `uv_vis_axis`, `raw_signal`, optional `reference_signal`, optional `numeric_corrected_signal`, optional `smoothed_signal`, `processed_signal`, and wavelength/energy conversions where units allow. When `numeric_correction.enabled` is reviewed and valid, EA writes `uv_vis_numeric_correction.yml` and applies the corrected signal before smoothing, normalization, and feature detection. When `tauc_analysis.enabled` is reviewed and valid, EA also writes `uv_vis_tauc.csv` with `tauc_energy_eV`, `tauc_alpha_proxy`, `tauc_y`, and `tauc_fit_window`. When `derivative_analysis.enabled` is reviewed and valid, EA writes `uv_vis_derivative.csv` with derivative axis, wavelength/energy coordinates, processed signal, first derivative, second derivative, method, and assignment source. When `correction_context.enabled` is reviewed, EA writes `uv_vis_correction_context.yml` with sample geometry, substrate, reference, background, diffuse-reflectance context, notes, confidence, source, boundary, and record ref.
...
- Reports include an embedded UV-Vis figure, original figure path, feature tables, optical edge estimate, optional reviewed numeric-correction summary/link, optional Tauc/Kubelka-Munk screening summary/table link, optional derivative screening summary/table link, confidence-labeled possible interpretations, file links, References, and provenance.
...
- Literature source-candidate staging supports `ea literature prepare-source-candidates --method uv_vis` and `ea literature preflight-source-candidates --method uv_vis` for confirmed local literature manifests. Candidate types are `optical_transition_model`, `optical_gap_candidate`, `optical_feature_assignment`, and `correction_context_candidate`. `ea uv-vis list-source-libraries` prints a local JSON discovery summary for built-in UV-Vis source libraries, with candidate counts, candidate types, optical targets, energy ranges, wavelength ranges, reference seeds, candidate summaries, filters, recommended next commands, and no-auto-application boundaries. `ea uv-vis build-source-packet` can write an editable template or copy built-in, reviewed local, or confirmed-literature candidates into traceable staging packets under `suggestions/uv_vis/source-packets/`. `ea uv-vis suggest-interpretations` consumes a processed UV-Vis metadata file plus a source packet and writes advisory `uv_vis_interpretation_suggestions.yml` / `.csv` under `suggestions/uv_vis/`, preserving source summary, applicability notes, confidence, caveats, registered/unresolved `reference_ids`, and links to processed feature/Tauc/edge/derivative/correction-context evidence where available. `ea uv-vis prepare-review` writes grouped `review_package.yml` / `.md` files for ready, unresolved-reference, no-evidence-match, and invalid candidates. `ea uv-vis report --interpretation-suggestion ... --interpretation-review-ref ...` can include reviewed source-backed candidates in the report, merge registered candidate references into `References`, and leave unresolved/no-match/invalid candidates visible as warnings/context. `ea uv-vis propose-memory --suggestion ... --review-ref ...` writes draft interpretation memory candidates from ready reviewed suggestions only; the standard memory review/commit flow is still required before anything becomes durable project memory. Discovery, source packets, suggestions, and review packages do not create ReviewRecords, inject report citations, auto-apply Tauc/Kubelka-Munk/correction models, or prove band gaps/transition mechanisms; report and memory proposal steps only read a confirmed ReviewRecord and registered references, and memory proposal does not commit confirmed memory.
- `ea uv-vis compare-replicates` reads two or more processed `uv_vis_metadata.yml` records and writes `uv_vis_comparison.yml` plus `uv_vis_comparison.csv` under `processed/comparisons/uv_vis/<comparison_id>/`. It summarizes sample/result refs, signal mode, x unit, optical edge estimates, Tauc intercept screening values, feature counts and feature positions, derivative/correction-context availability, warnings, review/provenance refs, and descriptive count/mean/std/min/max statistics only when the numeric screening values share a comparable basis. Missing or not-comparable metrics remain explicit. By default feature positions are listed per record and not matched. With `--feature-match-tolerance-ev` and/or `--feature-match-tolerance-nm` plus a confirmed `--feature-match-review-ref` targeting `uv_vis_feature_matching`, the comparison adds low-confidence reviewed tolerance groups with member feature IDs, metadata/result/sample refs, axis values, group statistics, duplicate-record warnings, and boundaries. These groups are candidate alignment aids only and do not prove feature assignments, transition mechanisms, band gaps, replicate equivalence, or sample ranking.
...
- `correction_context` is disabled by default. Enable it only after the user reviews substrate/reference/background/sample-geometry/diffuse-reflectance metadata. This record is metadata/provenance only; it does not apply numeric correction by itself.
- `numeric_correction` is disabled by default. Enable it only after the user reviews the correction operation and values. Current supported methods are `constant_offset` and `subtract_reference_column`; the latter requires the raw file to contain a reviewed third column such as `blank_absorbance`. Numeric correction is a transparent preprocessing step only; it does not choose references, fit substrate/background models, prove correction validity, or rank samples.
...
Built-in UV-Vis source starter library:

- Use `ea uv-vis list-source-libraries` before packet creation when the user or agent needs to inspect bundled source-backed optical interpretation coverage, candidate IDs, candidate types, optical targets, energy/wavelength ranges, reference seeds, filters, and next commands.
- Current built-in library: `generic_optical_interpretation`.
- Candidate types include `optical_transition_model`, `optical_gap_candidate`, `optical_feature_assignment`, and `correction_context_candidate`.
- Starter candidates cover direct/indirect Tauc-style screening context, a broad wide-band-gap oxide edge discussion window, visible sub-band-gap feature discussion, and diffuse-reflectance/Kubelka-Munk context. They are low-confidence discussion starters, not material-specific proof.
- Built-in candidates carry `reference_seeds`; use `ea references register-seeds` only when the user or agent explicitly wants those seeds registered in this project.
- Discovery reads bundled metadata only. It does not run live lookup, create project files, register references, build packets, generate suggestions, create ReviewRecords, inject citations, apply optical models/corrections, write memory, or prove band gaps, transition mechanisms, feature assignments, or correction validity.
- Use `--include-candidate`, `--candidate-type`, `--optical-target`, `--energy-min-ev`, `--energy-max-ev`, `--wavelength-min-nm`, and `--wavelength-max-nm` to narrow discovery; use `--builtin-library`, `--include-candidate`, `--candidate-type`, and `--optical-target` on `build-source-packet` to narrow packet creation.
...
Optional reviewed numeric correction can be supplied with `--parameters-json` or `--parameters-file`:
...
numeric_correction:
  ...
  constant_offset: 0.0
  correction_notes:
    - Blank/reference column and scaling were reviewed by the user before processing.
```

Future UV-Vis work should add richer substrate/reference/background correction models only after source/review/uncertainty semantics are designed, richer feature-matching/statistical models for replicate sets, richer material assignment libraries, and richer multi-candidate memory review helpers.
