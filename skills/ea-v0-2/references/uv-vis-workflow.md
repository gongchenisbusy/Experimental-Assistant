...
5. Record smoothing, normalization, feature detection, optional optical edge estimate, generated figure, report, and provenance.
...
7. Treat optical features and threshold edge estimates as screening hints. Use sample geometry, substrate/background, references, and user review before writing durable conclusions.
...
- Processing supports optional Savitzky-Golay smoothing, max-absolute normalization, SciPy optical feature detection, and a low-confidence normalized-threshold optical edge estimate.
- `signal_mode=absorbance` detects maxima; `signal_mode=transmittance` or `reflectance` detects minima.
- Processed CSV files include `uv_vis_axis`, `raw_signal`, optional `smoothed_signal`, `processed_signal`, and wavelength/energy conversions where units allow.
...
- Reports include an embedded UV-Vis figure, original figure path, feature tables, optical edge estimate, confidence-labeled possible interpretations, file links, References, and provenance.
...
Future UV-Vis work should add user-confirmed Tauc/derivative/Kubelka-Munk analysis modes, baseline/substrate/reference correction records, replicate comparison, diffuse-reflectance-specific metadata, richer material assignment libraries, and user-confirmed memory-candidate generation from report interpretations.
