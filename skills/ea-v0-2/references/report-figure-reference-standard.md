...
Use `ea export report-bundle /path/to/ea-project --report-id <report_id>` when a user or later agent needs the report, figure files, source data, result metadata, references, and provenance gathered into one local handoff folder. Use `ea export batch-bundle /path/to/ea-project --batch-id <batch_id>` when a whole batch run and its nested report bundles should travel together. Add `--zip` when the same handoff bundle should also be packaged as an archive.
