...
Use `ea export report-bundle /path/to/ea-project --report-id <report_id>` when a user or later agent needs the report, figure files, source data, result metadata, references, and provenance gathered into one local handoff folder. Add `--zip` when the same handoff bundle should also be packaged as an archive.
