...
Figure index records should include `style_profile` and `source_data_refs` when generated from processed data. Raman, PL, XRD, FTIR, UV-Vis, XPS, and electrochemistry workflows write these fields so another agent can trace a plotted figure back to processed CSV and peak/feature-table sources.
