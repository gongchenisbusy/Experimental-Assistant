...
- For publishing or handing off the Experimental Assistant v0.9.6 repository itself, run `ea-public-release-smoke` after project-level readiness checks.
- The smoke gate is broader than `ea eval project`: it runs the test suite, validates the Experimental Assistant v0.9.6 compatibility skill package, checks core CLI help commands, scans product files for accidental developer-machine defaults, and checks release-facing files for credential-like assignments or token literals.
