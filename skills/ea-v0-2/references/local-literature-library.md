...
`preflight-source-candidates` checks a confirmed FTIR/UV-Vis/XPS source-candidate manifest before the user builds or stages a source packet. It verifies confirmation, included candidates, required method fields, reference IDs, and available `reference_seeds`, then writes `<method>_source_candidates_preflight.yml`. A ready preflight means the manifest can be passed to `ea ftir build-assignment-packet --literature-manifest ...`, `ea xps build-source-packet --literature-manifest ...`, or future UV-Vis source-packet work. UV-Vis preflight currently validates source-backed staging only; it does not build UV-Vis source packets, register references, inject citations, apply optical models/corrections, download or parse full text, or prove band gaps/transition assignments.
