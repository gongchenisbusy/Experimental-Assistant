...
Use this reference when preparing the EA v0.9 release-candidate repository for public handoff or a release checkpoint.
...
- Writes `dist/ea-v0.9-rc-release-manifest.yml` by default.
...
- Writes `dist/ea-v0-2-0.9.0rc1-{commit}-release.zip` by default.
- Writes a `.zip.sha256` sidecar next to the archive.
- Stores all archive members under one top-level directory.
- Includes `ea-v0.9-rc-release-manifest.yml` inside the archive.
...
- Finds the embedded `ea-v0.9-rc-release-manifest.yml`.
