...
Built-in Experimental Assistant v0.9.5 compatibility manifest catalogue lives in `skill-registry/builtins/` and is indexed by `skill-registry/index.yml`. Current built-ins cover Raman, PL, XRD, FTIR, UV-Vis, XPS, electrochemistry, thermal analysis, image analysis, local literature library, and scientific figure generation. Raman, PL, XRD, FTIR, UV-Vis, XPS, electrochemistry, thermal analysis, image-data, scientific-figure style infrastructure, and local literature library planning/public-metadata-search/supplied-candidate ranking/import/status infrastructure have concrete initial workflows; full-text literature acquisition and deeper specialized analysis entries remain contract boundaries where implementation services are still intentionally limited or externalized behind user confirmation.
