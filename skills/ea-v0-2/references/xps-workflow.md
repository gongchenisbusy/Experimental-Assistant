...
- When `build-source-packet` is run, EA writes a standard `xps_parameter_source_packet.yml`-style YAML packet with source-library kind/ref, filters, candidate metadata, reference IDs, optional filtered `reference_seeds`, optional `guidance_reference_ids`, provenance, and no-auto-application boundaries.
