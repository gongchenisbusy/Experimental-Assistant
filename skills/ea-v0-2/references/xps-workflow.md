...
5. If source-backed XPS parameter suggestions are useful, EA may prepare candidate parameters from the built-in `generic_xps_parameters` seed library, a reviewed local library, project literature record, user-provided source, or user-confirmed literature/search workflow. This does not require every signed energy separation, area ratio, FWHM ratio, Tougaard parameter, or reviewed bound to be typed by the user; source-backed values can be proposed when element/region/sample context, source, and applicability are recorded. Use `ea xps build-source-packet` with no library arguments for the built-in starter packet, `--builtin-library generic_xps_parameters` for explicit built-in selection, `--library-file`/`--write-template` for project-local libraries and editable templates, or `--literature-manifest` for a confirmed literature/source-candidate manifest; then run `ea xps suggest-parameters` on the source packet before copying any values into processing parameters. Use `ea xps prepare-review` to create a grouped YAML/Markdown review package, then ask the user to review candidate values and register missing references.
