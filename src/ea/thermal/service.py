...
def _apply_baseline_correction(
    ...
    if method != "linear_two_point":
        warning = _warning(
            ...
            "Thermal baseline correction method is not supported by Experimental Assistant v0.9.6.",
            severity="medium",
            method=method,
        )
...
def _analyze_transitions(processed: pd.DataFrame, parameters: dict[str, Any], request: ThermalAnalysisProcessingRequest) -> tuple[pd.DataFrame, dict[str, Any] | None, list[dict[str, Any]]]:
    ...
    if method != "reviewed_window_screening":
        warning = _warning("thermal_transition_method_unsupported", "Thermal transition analysis method is not supported by Experimental Assistant v0.9.6.", severity="medium", method=method)
        warnings.append(warning)
        record.update({"status": "skipped_unsupported_method", "warnings": warnings})
...
def _record_transition_assignments(
    ...
    if method != "user_confirmed_transition_assignments":
        warning = _warning("thermal_transition_assignment_method_unsupported", "Thermal transition assignment method is not supported by Experimental Assistant v0.9.6.", severity="medium", method=method)
        warnings.append(warning)
        record.update({"status": "skipped_unsupported_method", "warnings": warnings})
