...
""".strip()
    ...
    (root / "dist" / "experimental-assistant-0.9.7-sbom.json").write_text(
        json.dumps(
            {
                "bomFormat": "CycloneDX",
                "specVersion": "1.5",
                "metadata": {
                    "component": {"name": "experimental-assistant", "version": "0.9.7"}
                },
                "components": [],
            }
        )
    ...
    (
        root / "dist" / "experimental-assistant-0.9.7-vulnerability-report.json"
    ).write_text(
        json.dumps({"scanner": "pip-audit", "status": "pass", "vulnerability_count": 0})
        + "\n",
        encoding="utf-8",
    )
    (root / "dist" / "experimental-assistant-0.9.7-install-smoke.json").write_text(
        json.dumps(
            {
                "status": "pass",
    ...
    wheel = root / "dist" / "experimental_assistant-0.9.7-py3-none-any.whl"
    sdist = root / "dist" / "experimental_assistant-0.9.7.tar.gz"
    wheel.write_bytes(b"wheel")
    sdist.write_bytes(b"sdist")
    (root / "dist" / "experimental-assistant-0.9.7-reproducibility.json").write_text(
        json.dumps(
            {
                "status": "pass",
                "distribution": "experimental-assistant",
