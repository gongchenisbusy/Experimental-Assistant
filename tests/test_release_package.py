...
def test_release_package_writes_zip_manifest_and_checksum_sidecar(tmp_path: Path) -> None:
    ...
    assert result["manifest_archive_ref"] == "ea-release/ea-v0.9.6-release-manifest.yml"

    with zipfile.ZipFile(archive_path) as archive:
        names = set(archive.namelist())
        assert "ea-release/ea-v0.9.6-release-manifest.yml" in names
        ...
        manifest = yaml.safe_load(archive.read("ea-release/ea-v0.9.6-release-manifest.yml"))

    assert manifest["manifest_type"] == "ea_v0_9_6_release"
...
def test_release_package_verifier_reports_missing_embedded_manifest(tmp_path: Path) -> None:
    ...
    _rewrite_zip(archive_path, skip_suffix="ea-v0.9.6-release-manifest.yml")
    Path(package["archive_checksum_path"]).write_text(_sha256(archive_path) + "  release.zip\n", encoding="utf-8")

    result = verify_release_package(archive_path)

    assert result["status"] == "fail"
    assert {"path": "ea-v0.9.6-release-manifest.yml", "reason": "missing_embedded_manifest"} in result["failures"]
