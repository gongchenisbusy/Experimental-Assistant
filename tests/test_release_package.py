...
def _rewrite_zip(
    ...
    *,
    ...
    path.replace(source)
    with (
        zipfile.ZipFile(source) as old_archive,
        zipfile.ZipFile(path, "w") as new_archive,
    ):
...
def _rewrite_current_package_as_v0_9_9(path: Path) -> None:
    source = path.with_suffix(".current.zip")
    path.replace(source)
    with (
        zipfile.ZipFile(source) as old_archive,
        zipfile.ZipFile(path, "w") as new_archive,
...
def test_release_package_writes_zip_manifest_and_checksum_sidecar(
    tmp_path: Path,
) -> None:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        assert "ea-release/examples/example_manifest.yml" in names
    ...
    assert manifest["manifest_type"] == "experimental_assistant_release"
    assert (
...
def test_release_package_is_deterministic_for_same_inputs(tmp_path: Path) -> None:
    ...
    first = write_release_package(
        root, output=Path("dist/first.zip"), archive_root="ea-release"
    )
    second = write_release_package(
        root, output=Path("dist/second.zip"), archive_root="ea-release"
    )
...
def test_v1_release_package_verifier_accepts_v0_9_9_manifest_type(
    tmp_path: Path,
) -> None:
    root = _minimal_release_root(tmp_path)
    package = write_release_package(
    ...
    _rewrite_current_package_as_v0_9_9(archive_path)
    ...
    assert result["status"] == "pass"
    assert result["check_type"] == "ea_v1_release_package"
    assert result["artifact_type"] == "ea_v0_9_9_release_package"
    assert result["artifact_version"] == "0.9.9"
    assert result["manifest_archive_ref"].endswith(
        "experimental-assistant-v0.9.9-release-manifest.yml"
    )
...
def test_release_package_verifier_reports_missing_sidecar(tmp_path: Path) -> None:
    ...
    assert {
        "path": package["archive_checksum_path"],
        "reason": "missing_checksum_sidecar",
    } in result["failures"]
...
def test_release_package_verifier_reports_archive_hash_mismatch(tmp_path: Path) -> None:
    ...
    assert result["status"] == "fail"
    assert any(
        failure["reason"] == "sha256_mismatch" and failure["path"] == "release.zip"
        for failure in result["failures"]
    )


def test_release_package_verifier_reports_missing_embedded_manifest(
    tmp_path: Path,
) -> None:
    root = _minimal_release_root(tmp_path)
    package = write_release_package(
...
def test_release_package_verifier_reports_payload_mismatch(tmp_path: Path) -> None:
    ...
    assert result["status"] == "fail"
    assert any(
        failure["path"] == "ea-release/README.md"
        and failure["reason"] == "size_mismatch"
        for failure in result["failures"]
    )
    assert any(
        failure["path"] == "ea-release/README.md"
        and failure["reason"] == "sha256_mismatch"
        for failure in result["failures"]
    )


def test_release_package_verify_cli_returns_nonzero_on_failure(
