...
def test_public_onboarding_doc_is_public_safe_and_actionable() -> None:
    ...
    assert "# Experimental Assistant v0.9.6 Public Onboarding" in text
...
def test_public_install_and_skill_setup_doc_is_public_safe_and_actionable() -> None:
    ...
    assert "Product identity: `Experimental Assistant v0.9.6` with package compatibility name `ea-v0-2`" in text
    assert "uv tool install --python 3.12 git+https://github.com/gongchenisbusy/Experimental-Assistant.git@v0.9.6" in text
    ...
    assert "https://github.com/gongchenisbusy/Experimental-Assistant/releases/tag/v0.9.6" in text
