...
def test_v0_9_release_candidate_docs_are_public_safe_and_actionable() -> None:
    ...
    assert "Experimental Assistant v0.9.5 Public Acceptance Matrix" in text_by_path[Path("docs/PUBLIC_ACCEPTANCE_MATRIX.md")]
    assert "Package version: `0.9.5`" in text_by_path[Path("docs/V0_9_RELEASE_NOTES.md")]
...
def test_v0_9_public_version_surfaces_do_not_look_like_v0_2_release() -> None:
    ...
    assert "initialize a public-user Experimental Assistant v0.9.5" in parser_help
...
def test_v0_9_release_candidate_docs_are_packaged(tmp_path: Path) -> None:
    ...
    assert manifest["release"]["label"] == "v0.9.5"
    ...
    assert manifest["public_repository"]["release_url"].endswith("/releases/tag/v0.9.5")
