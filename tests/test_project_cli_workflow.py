...
def test_cli_runs_public_raman_workflow_end_to_end(tmp_path: Path, capsys) -> None:
    ...
    assert (
        Path(project["project_working_memory"])
        .as_posix()
        .endswith("memory/project-working-memory.md")
    )
    ...
    assert (
        main(["raman", "inspect", str(workspace), raw_output["project_raw_path"]]) == 0
    )
    ...
    figure_record = read_yaml(workspace / "figures" / "index.yml")["figures"][
        raman["figure_id"]
    ]
    assert (
        figure_record["generation"]["parameters"]["plot_layout"]
        == "processed_main_raw_secondary_axis"
    )
    ...
    report_frontmatter, report_body = read_markdown_record(
        Path(report_output["report"])
    )
    ...
    updated_figure_record = read_yaml(workspace / "figures" / "index.yml")["figures"][
        raman["figure_id"]
    ]
    assert (
        updated_figure_record["footer"]
        == f"FigID: {raman['figure_id']} | Report: {report_frontmatter['report_id']}"
    )
    report_figure = workspace / figure_path_for_report(
        updated_figure_record, report_frontmatter["report_id"]
    )
    assert report_figure.is_file()
    assert report_figure.read_bytes() != figure_before_report
