...
from ea.release_distribution import (
    build_distribution_checklist,
    main,
    render_distribution_markdown,
    write_distribution_checklist,
)
...
def test_distribution_checklist_passes_for_manifest_and_package(tmp_path: Path) -> None:
    ...
    assert (
        checklist["release_artifacts"]["archives"][0]["verification"]["status"]
        == "pass"
    )
    assert (
        checklist["release_artifacts"]["archives"][0]["signature"]["status"]
        == "not_present"
    )
...
def test_distribution_checklist_prefers_current_version_archives(
    tmp_path: Path,
) -> None:
    root = _minimal_release_root(tmp_path)
    write_release_manifest(root)
    package = write_release_package(root)
    old_archive = root / "dist" / "experimental-assistant-0.2.0-old-release.zip"
    ...
    assert [archive["path"] for archive in archives] == [
        Path(package["archive_path"]).relative_to(root).as_posix()
    ]


def test_distribution_checklist_rejects_artifact_replaced_after_reproducibility_check(
    tmp_path: Path,
) -> None:
    root = _minimal_release_root(tmp_path)
    write_release_manifest(root)
    ...
    (root / "dist" / "experimental_assistant-1.0.0.tar.gz").write_bytes(
    ...
    failure = next(
        ...
        if item["code"] == "python_distribution.reproducibility"
    )
    assert failure["evidence"]["current_artifact_hashes_match"] is False


def test_distribution_checklist_verifies_optional_signature_when_public_key_is_supplied(
    tmp_path: Path,
) -> None:
...
def test_distribution_checklist_cli_writes_json_and_markdown(
    ...
    assert "Experimental Assistant v1.0.0 Distribution Checklist" in Path(
        summary["markdown_path"]
    ).read_text(encoding="utf-8")


def test_distribution_checklist_markdown_includes_required_checks(
    tmp_path: Path,
) -> None:
    root = _minimal_release_root(tmp_path)
    write_release_manifest(root)
...
def test_write_distribution_checklist_returns_paths_and_payload(tmp_path: Path) -> None:
    ...
    json_path, md_path, checklist = write_distribution_checklist(
        root, output_json=Path("dist/out.json"), output_md=Path("dist/out.md")
    )
